...
LOG_DIR="$BASE/logs"
RESULT_ROOT="$BASE/nnUNet_results/Dataset004_ATLAS3_RAW/nnUNetTrainer__nnUNetResEncUNetMPlans__3d_fullres"
TRAIN="${NNUNET_TRAIN_BIN:-nnUNetv2_train}"
MANAGER_LOG="$LOG_DIR/train_raw_resencm_folds1_4_queue_20260721.log"
LOCK_FILE="$LOG_DIR/train_raw_resencm_folds1_4_queue.lock"
...
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
    echo "A ResEncM folds 1-4 queue is already running."
    exit 1
fi
...
wait_for_gpu() {
    ...
    while true; do
        for gpu in 0 1 2 3; do
            ...
            # Require two consecutive checks to avoid launching during a brief gap.
...
log "serial queue started; only one ResEncM fold will run at a time"
log "waiting for a GPU with at least ${MIN_FREE_MIB} MiB free"

for fold in 1 2 3 4; do
    fold_dir="$RESULT_ROOT/fold_$fold"
    fold_log="$LOG_DIR/train_raw_resencm_fold${fold}_20260721.log"
    ...
    while (( attempt < MAX_ATTEMPTS )); do
        gpu=$(wait_for_gpu)
        attempt=$(( attempt + 1 ))
        command=("$TRAIN" 4 3d_fullres "$fold" -p nnUNetResEncUNetMPlans --npz)
...
log "all requested ResEncM folds completed"
