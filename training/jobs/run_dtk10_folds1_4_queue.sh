...
LOG_DIR="$BASE/logs"
RESULT_ROOT="$BASE/nnUNet_results/Dataset004_ATLAS3_RAW/nnUNetTrainerDiceTopK10Loss__nnUNetPlans__3d_fullres"
TRAIN="${NNUNET_TRAIN_BIN:-nnUNetv2_train}"
MANAGER_LOG="$LOG_DIR/train_raw_dtk10_folds1_4_queue_20260723.log"
LOCK_FILE="$LOG_DIR/train_raw_dtk10_folds1_4_queue.lock"
...
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
    echo "A DTK10 folds 1-4 queue is already running."
    exit 1
fi
...
log "serial queue started; only one DTK10 fold will run at a time"
log "waiting for a GPU with at least ${MIN_FREE_MIB} MiB free"

for fold in 1 2 3 4; do
    fold_dir="$RESULT_ROOT/fold_$fold"
    fold_log="$LOG_DIR/train_raw_dtk10_fold${fold}_20260723.log"
    ...
    while (( attempt < MAX_ATTEMPTS )); do
        gpu=$(wait_for_gpu)
        attempt=$(( attempt + 1 ))
        command=("$TRAIN" 4 3d_fullres "$fold" -tr nnUNetTrainerDiceTopK10Loss --npz)
...
log "all requested DTK10 folds completed"
