#!/usr/bin/env python3
"""Full-volume OOF evaluation of shortlisted three-model fusion settings.
...
DEFAULT_SELECTED_JSON = (
    BASE
    / "ensemble_results/three_model_weight_search_cv/"
    "selected_sample_candidates.json"
)
DEFAULT_OUT_DIR = BASE / "ensemble_results/three_model_weight_search_cv/full_oof"
MODEL_ORDER = ("resencm", "dtk10", "msl")
METRICS = (
    "dice",
    "lesion_f1",
...
MODES = (
    {"name": "raw", "small_limit_mm3": 0.0, "max_confidence": 0.0},
    {"name": "pp_s5_c055", "small_limit_mm3": 5.0, "max_confidence": 0.55},
    {"name": "pp_s10_c055", "small_limit_mm3": 10.0, "max_confidence": 0.55},
    {"name": "pp_s25_c055", "small_limit_mm3": 25.0, "max_confidence": 0.55},
    {"name": "pp_s5_c060", "small_limit_mm3": 5.0, "max_confidence": 0.60},
    {"name": "pp_s10_c060", "small_limit_mm3": 10.0, "max_confidence": 0.60},
    {"name": "pp_s25_c060", "small_limit_mm3": 25.0, "max_confidence": 0.60},
    {"name": "pp_s50_c055", "small_limit_mm3": 50.0, "max_confidence": 0.55},
    {"name": "pp_s100_c055", "small_limit_mm3": 100.0, "max_confidence": 0.55},
    {"name": "pp_s200_c055", "small_limit_mm3": 200.0, "max_confidence": 0.55},
    {"name": "pp_s50_c060", "small_limit_mm3": 50.0, "max_confidence": 0.60},
    {"name": "pp_s100_c060", "small_limit_mm3": 100.0, "max_confidence": 0.60},
    {"name": "pp_s200_c060", "small_limit_mm3": 200.0, "max_confidence": 0.60},
    {"name": "pp_s200_c065", "small_limit_mm3": 200.0, "max_confidence": 0.65},
)
...
def load_candidates(path: Path, max_candidates: int) -> list[dict]:
    ...
    add(
        {"resencm": 1.0 / 3.0, "dtk10": 1.0 / 3.0, "msl": 1.0 / 3.0},
        0.35,
        "mandatory_current_equal_baseline",
    )
    for item in source:
        add(item["weights"], item["threshold"], item.get("screen_reason", "shortlist"))
...
def metrics_for_kept_components(
    ...
    for bin_index, (name, _, _) in enumerate(SIZE_BINS_MM3):
        ...
        detected = sum(int(truth_id) in matched_truth for truth_id in truth_ids)
...
def summarize(results: list[dict], candidates: list[dict]) -> dict:
    ...
    baseline_by_mode = {}
    equal_candidate = next(
        candidate
        for candidate in candidates
        if candidate["reason"] == "mandatory_current_equal_baseline"
    )
    for mode in MODES:
        baseline_by_mode[mode["name"]] = (
            f"{equal_candidate['base_name']}__{mode['name']}"
        )
    ...
    raw_baseline_name = baseline_by_mode["raw"]
    raw_baseline_column = names.index(raw_baseline_name)
    strict_columns = []
    for column in range(len(names)):
        favorable = [
            (arrays[metric][:, column].mean() - arrays[metric][:, raw_baseline_column].mean())
            * DIRECTIONS[metric]
            for metric in METRICS
        ]
        if min(favorable) > 1e-12 and missed[:, column].sum() <= missed[:, raw_baseline_column].sum():
    ...
    for column, name in enumerate(names):
        comparison = {}
        for metric in METRICS:
            raw_delta = arrays[metric][:, column] - arrays[metric][:, raw_baseline_column]
        ...
        comparison["missed_case_delta"] = int(
            missed[:, column].sum() - missed[:, raw_baseline_column].sum()
        )
        paired_vs_raw[name] = comparison
